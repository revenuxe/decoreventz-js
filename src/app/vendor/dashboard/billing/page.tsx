  ...
  return (
    <section>
      <div className="mb-4">
        ...
        <p className="text-sm text-muted-foreground">Payments recorded against your billed orders.</p>
      </div>
        ...
        <>
          ...
          {filtered.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-border bg-card p-10 text-center text-sm text-muted-foreground">
              {bookings.length === 0 ? "No billed orders yet — admin sets a bill amount once your quote is reviewed." : "Nothing here."}
            </div>
          ) : (
      ...
      {selected && (
        ...
        >
          <div
            ...
            className="max-h-[85vh] w-full max-w-sm overflow-y-auto rounded-t-3xl bg-card shadow-elevated md:max-h-[80vh] md:rounded-3xl"
          >
            <div className="sticky top-0 z-10 flex items-center justify-between border-b border-border bg-card/95 p-4 backdrop-blur-lg">
              <p className="text-sm font-bold">#{selected.order_code}</p>
              <button
                onClick={() => setSelectedId(null)}
                aria-label="Close"
